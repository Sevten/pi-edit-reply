...
2. A **session tree** opens, listing all messages across every branch of the current conversation (same navigation as the built-in `/tree`):
   ...
   - `Enter` on a message opens the editor — assistant replies and your own **user messages** alike. Tool-call messages are editable only through their reasoning/text (the calls themselves are kept as-is); tool results and tool-call-only rounds (no thinking, no text — hidden, like native `/tree`) are not editable.
...
- **Tool calls survive**: edited copies keep their tool-call parts, so copied `toolResult` entries stay paired and the context remains valid. Clearing thinking and reply from a tool-call message is a valid edit ("keep the call, drop the prose"); clearing both from a plain message is rejected as "nothing left". Tool-call arguments and tool results are never editable.
