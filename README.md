...
3. Edit the content. Messages are prefilled as labeled sections whenever they contain reasoning:
   ...
   Edit either section (or both); deleting a section removes that part of the message. Messages without reasoning are prefilled as plain text. **Tool-call messages can be selected too** — e.g. to fix the reasoning that preceded a tool call; the tool calls are kept so the conversation tail stays coherent. In the editor: `Enter` submits the draft, `Shift+Enter`/`Ctrl+J` starts a new line, `Escape`/`Ctrl+C` **keeps the draft** and returns to the session tree (re-open the message to continue where you left off), `Ctrl+G` opens your system editor.
...
5. The **save dialog** opens with `Ctrl+S` in the tree (or `Escape` when there are pending edits). It lists every pending edit (before → after) and the commit options — there is no default, you always choose explicitly:
